...
namespace VRCTools.Common.Utils {
  ...
  public static class ArrayUtils {
    ...
    /// <summary>
    /// Creates a copy of the given array with the specified element appended at its end or creates a completely new
    /// array consisting of the given element if null is given as the source.
    /// </summary>
    /// <param name="source">a source array or null</param>
    /// <param name="element">an element</param>
    /// <typeparam name="T">an array type</typeparam>
    /// <returns>a new mutated array</returns>
    public static T[] AddElementOrCreate<T>(T[] source, T element) {
      if (source == null) {
      ...
      return AddElement(source, element);
    }
    ...
    public static T[] RemoveElement<T>(T[] source, int i) {
      if (source == null || source.Length == 0 || i >= source.Length) {
        return source;
      }
